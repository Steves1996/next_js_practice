...
export default function UpdateUserForm({ formId, formData, setFormData }) {
    ...
    if(isError) return <Bug message={"Error"}/>
    ...
    return (
        <form className="grid lg:grid-cols-2 w-4/6 gap-4" onSubmit={handleSubmit}>
            ...
            <div className="input-type">
                <input type="number" onChange={setFormData} defaultValue={salary} name="salary" className="border w-full px-5 py-3 focus:outline-none rounded-md" placeholder="Salary" />
            </div>
            <div className="input-type">
            ...
            <div className="flex gap-10 items-center">
                ...
                <div className="form-check">
                    <input type="radio" defaultChecked={status == "Inactive"} onChange={setFormData} value="Inactive" id="radioDefault2" name="status" className="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-green-500 checked:border-green-500 focus:outline-none duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" />
            ...
            <button className="flex justify-center text-md w-2/6 bg-yellow-400 text-white px-4 py-2 border rounded-md hover:bg-gray-50 hover:border-green-500 hover:text-green-500">
                Update <span className='px-2'><BiBrush size={24}></BiBrush></span>
            </button>
        </form>
    )
}